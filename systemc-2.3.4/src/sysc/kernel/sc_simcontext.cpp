...
namespace sc_core
{
    ...
    {
        std::string hierarchical_name = parent ? (std::string(parent->name()) + SC_HIERARCHY_CHAR) : "";
    ...
    sc_start(const sc_time &duration, sc_starvation_policy p)
    {
        ...
        if (context_p->m_prim_channel_registry->pending_updates() || !context_p->m_delta_events.empty())
        ...
        if (p == SC_RUN_TO_TIME && !context_p->m_paused && sim_status == SC_SIM_OK && context_p->m_curr_time < exit_time)
        {
